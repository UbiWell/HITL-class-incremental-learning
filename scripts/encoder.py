...
def extract_all_features(
    ...
    """
    Extract per-stream embeddings and stack into (N, DIM, D).
    ...
    stream_names      : list[str]  ordered stream names matching axis 2 of X
    batch_size        : int

    Returns
    -------
    Z : np.ndarray  shape (N, DIM, D)
        All streams share the same D if using one encoder.
        If encoders have different embed_dims, D is the max and shorter
        embeddings are zero-padded on the right — configure carefully.
    """
    N, T, DIM, C = X.shape
    assert len(stream_names) == DIM, (
        f"stream_names has {len(stream_names)} entries but X has DIM={DIM}"
    )
