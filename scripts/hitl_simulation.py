...
# ─────────────────────────────────────────────────────────────────────────────
# Mutual exclusion marking
# ─────────────────────────────────────────────────────────────────────────────

def simulate_mutual_exclusion_marking(
    new_activity: str,
    trained_activities: list[str],
    cfg: "Config",
) -> list[str]:
    """
    Simulate the user marking which trained activities are mutually exclusive
    with new_activity.

    In ground_truth mode: reads from mutual_exclusions in the dataset config.
    In 'none' mode: user never marks ME (tests robustness without ME).
    ...
    Returns
    -------
    exclusive : list of trained activities marked as mutually exclusive
    """
    sim_cfg = cfg.HITL_SIMULATION
    source  = sim_cfg.get("me_source", "ground_truth")

    if source == "none":
        print(f"\n  ME simulation disabled (me_source='none')")
        return []

    # ground_truth
    exclusive = cfg.get_mutual_exclusions_for(new_activity, trained_activities)
    if exclusive:
        print(f"\n  User marks ME: '{new_activity}' ✗↔ {exclusive}")
    else:
        print(f"\n  No mutual exclusions for '{new_activity}'")
    return exclusive
